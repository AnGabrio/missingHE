#' An internal function to write BUGS code for bivariate beta-normal with an hurdle model for the effects (independence)
#'
#' This function writes in the current WD a txt file 
#' for the independent joint beta-normal with an hurdle model for the effects.
...
beta_normal_hurdle_e_ind<-function(type)eval.parent(substitute({
  ...
  model_string_jags<-prior_hurdle(type=type,dist_e ="beta",dist_c="norm")
      ...
      writeLines(model_string_jags,"beta_normal_hurdle_e_ind.txt")
      model_string<-"beta_normal_hurdle_e_ind.txt"
  return(model_string)
}))
