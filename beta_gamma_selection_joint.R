#' An internal function to write BUGS code for bivariate beta-gamma selection model (joint)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate joint beta-gamma selection model for both effects and costs.
...
beta_gamma_selection_joint<-function(type)eval.parent(substitute({
  ...
  #call prior_change function to make changes to prior values and distributions if prior inputs in run_model are provided
  model_string_jags<-prior_selection(type=type,dist_e ="beta",dist_c="gamma")
      #assign name to model file to be called by run_jags or run_bugs
      writeLines(model_string_jags,"beta_gamma_selection_joint.txt")
      model_string<-"beta_gamma_selection_joint.txt"
  return(model_string)
}))
