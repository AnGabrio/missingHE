...
prior_selection<-function(type,dist_e,dist_c)eval.parent(substitute({
  ...
  if(type=="MNAR"|type=="MNAR_eff"|type=="MNAR_cost"){
    ...
    if(pe==1){
      if(is.null(mu.prior.e)==FALSE & grepl("beta_e[1]~",model_string_jags,fixed = T)==TRUE & grepl("beta_e[2]~",model_string_jags,fixed = T)==TRUE){
      ...
      if(is.null(mu.prior.e)==FALSE & grepl("beta_e[1,1]~",model_string_jags,fixed = T)==TRUE & grepl("beta_e[1,2]~",model_string_jags,fixed = T)==TRUE){
        if(length(mu.prior.e)!=2){stop("provide correct hyper prior values")}
        prior_mue<-mu.prior.e
    ...
    if(pc==1){
      if(is.null(mu.prior.c)==FALSE & grepl("beta_c[1]~",model_string_jags,fixed = T)==TRUE & grepl("beta_c[2]~",model_string_jags,fixed = T)==TRUE){
      ...
      if(is.null(mu.prior.c)==FALSE & grepl("beta_e[1,1]~",model_string_jags,fixed = T)==TRUE & grepl("beta_e[1,2]~",model_string_jags,fixed = T)==TRUE){
        if(length(mu.prior.c)!=2){stop("provide correct hyper prior values")}
        prior_muc<-mu.prior.c
  ...
  if(dist_e=="norm"){
  if(is.null(alpha.prior.e)==FALSE & grepl("ls_e[t]~",model_string_jags,fixed = T)==TRUE){
    ...
    model_string_jags<-gsub("ls_e[t]~dunif(-5,10", prior_alphae_str, model_string_jags,fixed=TRUE)}
      ...
      if(is.null(alpha.prior.e)==FALSE & grepl("s_e[t]~",model_string_jags,fixed = T)==TRUE){
        if(length(alpha.prior.e)!=2){stop("provide correct hyper prior values")}
        prior_alphae<-alpha.prior.e
  ...
  if(dist_c=="norm"){
    if(is.null(alpha.prior.c)==FALSE & grepl("ls_c[t]~",model_string_jags,fixed = T)==TRUE){
      ...
      if(is.null(alpha.prior.c)==FALSE & grepl("s_c[t]~",model_string_jags,fixed = T)==TRUE){
        if(length(alpha.prior.c)!=2){stop("provide correct hyper prior values")}
        prior_alphac<-alpha.prior.c
    ...
    #correlation only for joint models
      ...
      if(is.null(theta.prior)==FALSE & grepl("theta",model_string_jags,fixed = T)==TRUE){
        if(length(theta.prior)!=2){stop("provide correct hyper prior values")}
          ...
          model_string_jags<-gsub("theta[t]~dnorm(0,0.001", prior_theta_str, model_string_jags,fixed=TRUE)
        }
      }
