#' An internal function to write BUGS code for bivariate normal-gamma with an hurdle model for the effects (independence)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate independent normal-gamma with an hurdle model for the effects.
...
normal_gamma_hurdle_e_ind<-function(type)eval.parent(substitute({
  ...
  model_string_jags<-prior_hurdle(type=type,dist_e ="norm",dist_c="gamma")
      ...
      writeLines(model_string_jags,"normal_gamma_hurdle_e_ind.txt")
      model_string<-"normal_gamma_hurdle_e_ind.txt"
  return(model_string)
}))
