#' An internal function to write BUGS code for bivariate normal-gamma hurdle model (joint)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate joint normal-gamma hurdle model for both effects and costs.
...
#'  and Structural At Random (MNAR). It is also possible to explore some Structural Not At Random (SNAR) assumptions
#'  by providing a structural indicator variable for the missing cases for both effects and costs 
#' @examples
...
normal_gamma_hurdle_ec_joint<-function(type)eval.parent(substitute({
  ...
  model_string_jags<-prior_hurdle(type=type,dist_e ="norm",dist_c="gamma")
      ...
      writeLines(model_string_jags,"normal_gamma_hurdle_ec_joint.txt")
      model_string<-"normal_gamma_hurdle_ec_joint.txt"
  return(model_string)
}))
