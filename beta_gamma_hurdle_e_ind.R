#' An internal function to write BUGS code for bivariate beta-gamma with an hurdle model for the effects (independence)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate independent beta-gamma with an hurdle model for the effects.
...
beta_gamma_hurdle_e_ind<-function(type)eval.parent(substitute({
  ...
  if(type=="SCAR"){
    ...
    #mean regression
    log(mu_c1[i])<-inprod(X1_c[i,],beta_c1[])
    ...
    #mean regression
    log(mu_c2[i])<-inprod(X2_c[i,],beta_c2[])
  ...
  model_string_jags<-prior_hurdle(type=type,dist_e ="beta",dist_c="gamma")
      ...
      writeLines(model_string_jags,"beta_gamma_hurdle_e_ind.txt")
      model_string<-"beta_gamma_hurdle_e_ind.txt"
  return(model_string)
}))
