#' An internal function to write BUGS code for bivariate beta-gamma hurdle model (joint)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate joint beta-gamma hurdle model for both effects and costs.
...
beta_gamma_hurdle_ec_joint<-function(type)eval.parent(substitute({
  ...
  model_string_jags<-prior_hurdle(type=type,dist_e ="beta",dist_c="gamma")
      ...
      writeLines(model_string_jags,"beta_gamma_hurdle_ec_joint.txt")
      model_string<-"beta_gamma_hurdle_ec_joint.txt"
  return(model_string)
}))
