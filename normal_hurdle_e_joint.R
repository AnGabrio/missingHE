#' An internal function to write BUGS code for bivariate normal with an hurdle model for the effects (joint)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate joint normal with an hurdle model for the effects
...
normal_hurdle_e_joint<-function(type)eval.parent(substitute({
  ...
  if(type=="SCAR"){
    ...
    tau_c1<-1/psi_c1
    psi_c1<-ss_c1-ss_e1[1]*pow(theta[1],2)
    ss_c1<-s_c1*s_c1
    s_c1<-exp(ls_c1)
    tau_c2<-1/psi_c2
    psi_c2<-ss_c2-ss_e2[1]*pow(theta[2],2)
    ss_c2<-s_c2*s_c2
    s_c2<-exp(ls_c2)
  ...
  writeLines(model_string_jags,"normal_hurdle_e_joint.txt")
  model_string<-"normal_hurdle_e_joint.txt"
  return(model_string)
  }))
