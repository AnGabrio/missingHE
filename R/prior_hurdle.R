...
prior_hurdle <- function(type, dist_e, dist_c, pe, pc, ze, zc, se, sc) eval.parent( substitute( {
  if(type == "SCAR") {
    if(is.null(se) == FALSE) {
      if(is.null(gamma0.prior.e) == FALSE & grepl("gamma_e[1] ~ ", model_string_jags, fixed = TRUE) == TRUE) {
    ...
    if(is.null(sc) == FALSE) {
    if(is.null(gamma0.prior.c) == FALSE & grepl("gamma_c[1] ~ ", model_string_jags, fixed = TRUE) == TRUE) {
