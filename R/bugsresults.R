...
#' @keywords summary BUGS model
#' @param x The \code{bugs}, \code{oldClass}, or \code{mcmc.list} object
...
bugsresults <-function(x, params, regex=FALSE, invert=FALSE, 
  ...
  if(!isTRUE(requireNamespace("methods"))) {
    ...
    if(any(is(x) %in% c('oldClass', 'bugs'))) {
      nm <- dimnames(x$sims.array)[[3]]
      i <- grep(params, nm, invert=invert, ...)
      if(length(i) == 0) stop("No parameters match 'params'", call.=FALSE)
      samp <- x$sims.array[, , i, drop=FALSE] 
      rhat_neff <- x$summary[i, c('Rhat', 'n.eff'), drop=FALSE]
      out <- cbind(t(apply(
        samp, 3, function(x) 
      ...
      stop("x must be an 'mcmc.list' or 'bugs'  object.")
    }
    if(!missing(signif)) {
