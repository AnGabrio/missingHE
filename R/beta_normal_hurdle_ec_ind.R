#' An internal function to write BUGS code for bivariate beta-normal hurdle model (independence)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate independent beta-normal hurdle model for both effects and costs.
...
beta_normal_hurdle_ec_ind<-function(type)eval.parent(substitute({
  ...
  model_string_jags<-prior_hurdle(type=type,dist_e ="beta",dist_c="norm")
      ...
      writeLines(model_string_jags,"beta_normal_hurdle_ec_ind.txt")
      model_string<-"beta_normal_hurdle_ec_ind.txt"
  return(model_string)
}))
