#' An internal function to write BUGS code for bivariate beta-normal selection model (joint)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate joint beta-normal selection model for both effects and costs.
...
beta_normal_selection_joint<-function(type)eval.parent(substitute({
  ...
  model_string_jags<-prior_selection(type=type,dist_e ="beta",dist_c="norm")
      ...
      writeLines(model_string_jags,"beta_normal_selection_joint.txt")
      model_string<-"beta_normal_selection_joint.txt"
  return(model_string)
}))
