...
run_selection<-function(type,dist_e,dist_c,inits)eval.parent(substitute({
  ...
  model<-write_selection(type = type ,dist_e = dist_e,dist_c = dist_c)
  ...
  #define model output list
    if(type=="MAR"){
      model_output_jags<-list("summary"=model_sum,"model summary"=modelN1,"mean_effects"=mu_e,"mean_costs"=mu_c,"sd_effects"=s_e,"sd_costs"=s_c,
                              "covariate_parameter_effects"=beta_e,"covariate_parameter_costs"=beta_c,"missingness_probability_effects"=p_e,
