#' An internal function to write BUGS code for bivariate normal-gamma selection model (joint)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate joint normal-gamma selection model for both effects and costs.
...
normal_gamma_selection_joint<-function(type)eval.parent(substitute({
  ...
  model_string_jags<-prior_selection(type=type,dist_e ="norm",dist_c="gamma")
      ...
      writeLines(model_string_jags,"normal_gamma_selection_joint.txt")
      model_string<-"normal_gamma_selection_joint.txt"
  return(model_string)
}))
