#' An internal function to write BUGS code for bivariate beta-gamma with an hurdle model for the costs (independence)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate independent beta-gamma with an hurdle model for the costs.
...
beta_gamma_hurdle_c_ind<-function(type)eval.parent(substitute({
  ...
  if(type=="SCAR"){
    ...
    #mean regression
    log(mu_c1[i])<-inprod(X1_c[i,],beta_c1[,d_cost1[i]+1])
    ...
    #mean regression
    log(mu_c2[i])<-inprod(X2_c[i,],beta_c2[,d_cost2[i]+1])
