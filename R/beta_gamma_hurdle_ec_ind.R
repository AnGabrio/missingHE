#' An internal function to write BUGS code for bivariate beta-gamma hurdle model (independence)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate independent beta-gamma hurdle model for both effects and costs.
...
beta_gamma_hurdle_ec_ind<-function(type)eval.parent(substitute({
  ...
  model_string_jags<-prior_hurdle(type=type,dist_e ="beta",dist_c="gamma")
      ...
      writeLines(model_string_jags,"beta_gamma_hurdle_ec_ind.txt")
      model_string<-"beta_gamma_hurdle_ec_ind.txt"
  return(model_string)
}))
