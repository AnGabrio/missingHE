#' An internal function to write BUGS code for bivariate beta-normal selection model (independence)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate independent beta-normal selection model for both effects and costs.
...
beta_normal_selection_ind<-function(type)eval.parent(substitute({
  ...
  if(type=="MAR"){
    ...
    mu_e[1]<-ilogit(inprod(mean_cov_e1[],beta_e[,1]))
    mu_e[2]<-ilogit(inprod(mean_cov_e2[],beta_e[,2]))
  ...
  #call prior_change function to make changes to prior values and distributions if prior inputs in run_model are provided
  model_string_jags<-prior_selection(type=type,dist_e ="beta",dist_c="norm")
      #assign name to model file to be called by run_jags or run_bugs
      writeLines(model_string_jags,"beta_normal_selection_ind.txt")
      model_string<-"beta_normal_selection_ind.txt"
  return(model_string)
}))
