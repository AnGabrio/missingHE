...
#' @param model.me A formula expression in conventional R linear modelling syntax.  The response must be indicated with the 
...
#' \deqn{logit(\pi_i)=\gamma_0+\sum\gamma_j X^*_j +\delta(y)}
#' where
#' \itemize{
...
#' \item \eqn{\gamma_j} represents the impact on the probability of a missing value in \eqn{y} of the centered covariates \eqn{X^*_j}.
#' \item \eqn{\delta} represents the impact on the probability of a missing value in \eqn{y} of the missing value itself
#' }
#' 
#' When \eqn{\delta=0} the model assumes a 'MAR' mechanism, while when \eqn{\delta!=0} the mechanism is 'MNAR'.
#' 
...
#' in the function \code{selection}. Specifically, the default prior distributions that can be overwritten by the user are the following:
#' \itemize{
...
#' For the parameters indexing the missingness model, the default prior distributions assumed are the following:
#' \itemize{
...
#'x<-selection(data=data,model.eff=e~1,model.cost=c~1,model.me=me~1,model.mc=mc~1,
#'dist_e="norm",dist_c="norm",type="MAR")
...
selection<-function(data,model.eff,model.cost,model.me=me~1,model.mc=mc~1,dist_e,dist_c,type,prob=c(0.05,0.95),
  ...
  #covariates for e and c models (vectors of 1s if no variables)
  ...
  #covariates for me and mc models (vectors of 1s if no variables)
  ...
  #stop if MNAR selected and e and/or c provided in the me/mc models 
  #
  #default values for ind
  ...
  if(any(prior=="default")==TRUE){
    ...
    par_prior<-c("beta0.prior.e","beta0.prior.c","sigma.prior.e","sigma.prior.c","gamma.prior.e","gamma.prior.c",
                 "beta.prior.e","beta.prior.c","gamma0.prior.e","gamma0.prior.c","delta.prior.e","delta.prior.c","rho.prior")
    #stop message
    stop_mes<-"priors can be assigned only using specific string parameter names depending on the type of model assumed. 
    Type ''help(selection)'' for more details"
  ...
  #stop if delta_e or delta_c not present but prior provided
  ...
  #call run_hurdle to run the model in JAGS for each possible model setup 
  ...
  #unlink/remove file in directory for JAGS
  if(save_model==FALSE){
    unlink(filein)
  }
  #perform CEA calling BCEA
