...
#' pic.dic
#' ppc(model.selection, type = "histogram", ndisplay = 5)
#' #
#' 
#' \dontshow{
...
selection <- function(data, model.eff, model.cost, model.me = me ~ 1, model.mc = mc ~ 1, dist_e, dist_c, type, prob = c(0.05, 0.95), 
  ...
  data_read <- data_read_selection(data = data, model.eff = model.eff, model.cost = model.cost, model.me = model.me, model.mc = model.mc, type = type, center = center)
  ...
  formula.me <- all.vars(model.me)
  formula.me.length <- length(formula.me)
  formula.mc <- all.vars(model.mc)
  formula.mc.length <- length(formula.mc)
  ...
  if(length(which(is.na(c(eff1, eff2)))) == 0) {
    if(formula.me.length != 1 | formula.me[1] != "me") {
  ...
  if(length(which(is.na(c(cost1, cost2)))) == 0) {
    if(formula.mc.length != 1 | formula.mc[1] != "mc") {
