#' An internal function to write BUGS code for bivariate normal with an hurdle model for the costs (joint)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate joint normal with an hurdle model for the costs.
...
normal_hurdle_c_joint<-function(type)eval.parent(substitute({
  ...
  model_string_jags<-prior_hurdle(type=type,dist_e ="norm",dist_c="norm")
      ...
      writeLines(model_string_jags,"normal_hurdle_c_joint.txt")
      model_string<-"normal_hurdle_c_joint.txt"
  return(model_string)
}))
