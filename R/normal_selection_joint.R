#' An internal function to write BUGS code for bivariate normal selection model (joint)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate joint normal selection model for both effects and costs.
...
normal_selection_joint<-function(type)eval.parent(substitute({
  ...
  model_string_jags<-prior_selection(type=type,dist_e ="norm",dist_c="norm")
      ...
      writeLines(model_string_jags,"normal_selection_joint.txt")
      model_string<-"normal_selection_joint.txt"
  return(model_string)
}))
