#' An internal function to write BUGS code for bivariate beta-normal with an hurdle model for the costs (independence)
#'
#' This function writes in the current WD a txt file 
#' for the independent joint beta-normal with an hurdle model for the costs.
...
beta_normal_hurdle_c_ind<-function(type)eval.parent(substitute({
  ...
  if(type=="SCAR"){
    ...
    #mean regression
    mu_c1[i]<-inprod(X1_c[i,],beta_c1[,d_cost1[i]+1])
    ...
    #mean regression
    mu_c2[i]<-inprod(X2_c[i,],beta_c2[,d_cost2[i]+1])
  ...
  model_string_jags<-prior_hurdle(type=type,dist_e ="beta",dist_c="norm")
      ...
      writeLines(model_string_jags,"beta_normal_hurdle_c_ind.txt")
      model_string<-"beta_normal_hurdle_c_ind.txt"
  return(model_string)
}))
