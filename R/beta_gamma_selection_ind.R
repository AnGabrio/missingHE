#' An internal function to write BUGS code for bivariate beta-gamma selection model (independence)
#'
...
beta_gamma_selection_ind<-function(type)eval.parent(substitute({
  ...
  model_string_jags<-prior_selection(type=type,dist_e ="beta",dist_c="gamma")
      ...
      writeLines(model_string_jags,"beta_gamma_selection_ind.txt")
      model_string<-"beta_gamma_selection_ind.txt"
  return(model_string)
}))
