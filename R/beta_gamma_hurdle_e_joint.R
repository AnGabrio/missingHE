#' An internal function to write BUGS code for bivariate beta-gamma with an hurdle model for the effects (joint)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate joint beta-gamma with an hurdle model for the effects.
...
beta_gamma_hurdle_e_joint<-function(type)eval.parent(substitute({
  ...
  if(type=="SCAR"){
    ...
    tau_c1[i]<-mu_c1[i]/pow(s_c1,2)
    #mean regression
    log(mu_c1[i])<-inprod(X1_c[i,],beta_c1[])+theta[1]*(eff1[i]-mu_e[1])
    ...
    tau_c2[i]<-mu_c2[i]/pow(s_c2,2)
    #mean regression
    log(mu_c2[i])<-inprod(X2_c[i,],beta_c2[])+theta[2]*(eff2[i]-mu_e[2])
    ...
    nu_c[1]<-exp(inprod(mean_cov_c1[],beta_c1[]))
    nu_c[2]<-exp(inprod(mean_cov_c2[],beta_c2[]))
  ...
  model_string_jags<-prior_hurdle(type=type,dist_e ="beta",dist_c="gamma")
      ...
      writeLines(model_string_jags,"beta_gamma_hurdle_e_joint.txt")
      model_string<-"beta_gamma_hurdle_e_joint.txt"
  return(model_string)
}))
