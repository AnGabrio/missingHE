...
#' @param time_plot Time point for which plots should be displayed (only for long models).  
...
#' cost-effectiveness model implemented using the function \code{\link{selection}}, \code{\link{pattern}}, \code{\link{hurdle}} or or \code{\link{long_miss}}. 
...
plot.missingHE <- function(x, prob = c(0.025, 0.975), class = "scatter", outcome = "all", time_plot = NULL, theme = NULL, ...) {
  ...
  if(x$data_format == "long") {
    ...
    n_mis_comparator_e <- n_mis_comparator_c <- rep(NA, max_time) 
    n_mis_reference_e <- n_mis_reference_c <- rep(NA, max_time) 
    for(time in 1:max_time) {
      n_mis_comparator_e[time] <- x$data_set$`N missing in comparator arm`[1, time]
      n_mis_comparator_c[time] <- x$data_set$`N missing in comparator arm`[2, time]
      n_mis_reference_e[time] <- x$data_set$`N missing in reference arm`[1, time]
      n_mis_reference_c[time] <- x$data_set$`N missing in reference arm`[2, time]
    }
    if(all(n_mis_comparator_e) == 0 & all(n_mis_comparator_c) == 0 & all(n_mis_reference_e) == 0 & all(n_mis_reference_c) == 0) {
      stop("No missing value found in the data")
    }
    ...
    if(outcome == "all" & is.null(time_plot) == TRUE) {
      check_missing_e <- matrix(NA, nrow = 2, ncol = max_time)
      check_missing_c <- matrix(NA, nrow = 2, ncol = max_time)
      for(time in 1:max_time) {
        suppressWarnings(check_missing_e[ , time] <- c(any(is.na(x$data_set$effects[[1]][, time])), any(is.na(x$data_set$effects[[2]][, time]))))
        suppressWarnings(check_missing_c[ , time] <- c(any(is.na(x$data_set$costs[[1]][, time])), any(is.na(x$data_set$costs[[2]][, time]))))
      }
      if(any(c(check_missing_e, check_missing_c) == TRUE)) {n <- match(TRUE, c(check_missing_e, check_missing_c)) } else {n = 1 }
      ...
      check_missing_e <- matrix(NA, nrow = 2, ncol = 1)
      check_missing_c <- matrix(NA, nrow = 2, ncol = 1)
      suppressWarnings(check_missing_e[ , 1] <- c(any(is.na(x$data_set$effects[[1]][, time_plot])), any(is.na(x$data_set$effects[[2]][, time_plot]))))
      ...
      if(any(c(check_missing_e) == TRUE)) {n <- match(TRUE, c(check_missing_e)) } else {n = 1 }
      ...
      check_missing_e <- matrix(NA, nrow = 1, ncol = max_time)
      check_missing_c <- matrix(NA, nrow = 1, ncol = max_time)
      for(time in 1:max_time) {
        suppressWarnings(check_missing_e[ , time] <- c(any(is.na(x$data_set$effects[[1]][, time]))))
      ...
      check_missing_e <- matrix(NA, nrow = 1, ncol = 1)
      check_missing_c <- matrix(NA, nrow = 1, ncol = 1)
      suppressWarnings(check_missing_e[ , 1] <- c(any(is.na(x$data_set$effects[[1]][, time_plot]))))
      ...
      check_missing_c <- matrix(NA, nrow = 1, ncol = max_time)
      for(time in 1:max_time) {
        suppressWarnings(check_missing_e[ , time] <- c(any(is.na(x$data_set$effects[[2]][, time]))))
      ...
      check_missing_e <- matrix(NA, nrow = 1, ncol = 1)
      check_missing_c <- matrix(NA, nrow = 1, ncol = 1)
      suppressWarnings(check_missing_e[ , 1] <- c(any(is.na(x$data_set$effects[[2]][, time_plot]))))
