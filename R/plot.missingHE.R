...
#' @param x A \code{missingHE} object containing the results of the Bayesian modelling and the economic evaluation
...
#' @param class Type of plot comparing the observed and imputed outcome data. Available choices are
#' 'histogram' and 'scatter' for a histogram or a scatter plot of the observed and imputed outcome data, respectively.
#' @param outcome For which outcome type the plots should be displayed. Options are: 'all' (default) shows the plots
...
#' @seealso \code{\link{run_model}} \code{\link{diagnostic_checks}}
...
plot.missingHE<-function(x,prob=c(0.05,0.95),class="scatter",outcome="all",theme="base",...){
  ...
  if(x$model_class=="forward"){
    stop("missing plot not available for forward sampling output")
  }
