#' An internal function to write BUGS code for bivariate beta-normal with an hurdle model for the costs (joint)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate joint beta-normal with an hurdle model for the costs.
...
beta_normal_hurdle_c_joint<-function(type)eval.parent(substitute({
  ...
  model_string_jags<-prior_hurdle(type=type,dist_e ="beta",dist_c="norm")
      ...
      writeLines(model_string_jags,"beta_normal_hurdle_c_joint.txt")
      model_string<-"beta_normal_hurdle_c_joint.txt"
  return(model_string)
}))
