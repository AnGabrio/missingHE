#' An internal function to write BUGS code for bivariate normal-gamma with an hurdle model for the effects (joint)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate joint normal-gamma with an hurdle model for the effects.
...
normal_gamma_hurdle_e_joint<-function(type)eval.parent(substitute({
  ...
  model_string_jags<-prior_hurdle(type=type,dist_e ="norm",dist_c="gamma")
      ...
      writeLines(model_string_jags,"normal_gamma_hurdle_e_joint.txt")
      model_string<-"normal_gamma_hurdle_e_joint.txt"
  return(model_string)
}))
