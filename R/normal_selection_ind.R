#' An internal function to write BUGS code for bivariate normal selection model (independence)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate independent normal selection model for both effects and costs.
...
normal_selection_ind<-function(type)eval.parent(substitute({
  ...
  #call prior_change function to make changes to prior values and distributions if prior inputs in run_model are provided
  model_string_jags<-prior_selection(type=type,dist_e ="norm",dist_c="norm")
      #assign name to model file to be called by run_jags or run_bugs
      writeLines(model_string_jags,"normal_selection_ind.txt")
      model_string<-"normal_selection_ind.txt"
  return(model_string)
}))
