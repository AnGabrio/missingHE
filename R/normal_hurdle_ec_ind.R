#' An internal function to write BUGS code for bivariate normal hurdle model (independence)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate independent normal hurdle model for both effects and costs.
...
normal_hurdle_ec_ind<-function(type)eval.parent(substitute({
  ...
  model_string_jags<-prior_hurdle(type=type,dist_e ="norm",dist_c="norm")
      ...
      writeLines(model_string_jags,"normal_hurdle_ec_ind.txt")
      model_string<-"normal_hurdle_ec_ind.txt"
  return(model_string)
}))
