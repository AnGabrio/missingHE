#' An internal function to write BUGS code for bivariate normal hurdle model (joint)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate joint normal hurdle model for both effects and costs.
...
normal_hurdle_ec_joint<-function(type)eval.parent(substitute({
  ...
  if(type=="SCAR"){
    ...
    tau_c1[i]<-1/psi_c1[i]
    psi_c1[i]<-ss_c1[d_cost1[i]+1]-ss_e1[d_eff1[i]+1]*pow(theta1[d_cost1[i]+1],2)
    ...
    tau_c2[i]<-1/psi_c2[i]
    psi_c2[i]<-ss_c2[d_cost2[i]+1]-ss_e2[d_eff2[i]+1]*pow(theta2[d_cost2[i]+1],2)
  ...
  writeLines(model_string_jags,"normal_hurdle_ec_joint.txt")
  model_string<-"normal_hurdle_ec_joint.txt"
  return(model_string)
  }))
