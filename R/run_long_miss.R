#' An internal function to execute a JAGS longitudinal model and get posterior results
#'
...
#' @keywords JAGS Bayesian longitudinal models 
#' @examples
...
run_long_miss <- function(type, dist_e, dist_c, inits, ppc) eval.parent(substitute( {
  ...
  model <- write_long_miss(type = type , dist_e = dist_e, dist_c = dist_c, pe_fixed = pe_fixed, pc_fixed = pc_fixed, ze_fixed = ze_fixed, zc_fixed = zc_fixed, ind_fixed = ind_fixed,
                                ind_time_fixed = ind_time_fixed, pe_random = pe_random, pc_random = pc_random, ze_random = ze_random, zc_random = zc_random, ind_random = ind_random, 
  ...
  filein <- model
  datalist <- list("N1", "N2", "eff1", "eff2", "cost1", "cost2", "m_eff1", "m_eff2", "m_cost1", "m_cost2", "max_time", "npatt_e1", "npatt_e2", "npatt_c1", "npatt_c2",
  ...
  DIC <- TRUE
  params <- c("eff1", "eff2", "cost1", "cost2", "mu_e", "mu_c", "s_e", "s_c", "p_e", "p_c", "beta", "alpha", 
              "gamma_e1", "gamma_e2", "gamma_c1", "gamma_c2", "delta_e1", "delta_e2", "delta_c1", "delta_c2")
  ...
  if(time_dep == "AR1") {params <- c(params, "beta_te", "beta_tc", "alpha_te", "alpha_tc") }
  if(length(model_e_random) != 0){params <- c(params, "a1", "a2") }
  if(length(model_e_random) != 0 & ind_time_fixed == FALSE & time_dep == "AR1"){params <- c(params, "a1_te", "a2_te", "a1_tc", "a2_tc") }
  ...
  if(length(model_c_random) != 0 & is_c_random_c == FALSE & ind_time_fixed == FALSE & time_dep == "AR1"){
    params <- c(params, "b1_te", "b1_tc", "b2_te", "b2_tc") 
  }
  if(length(model_c_random) != 0 & is_c_random_c == TRUE & ind_time_fixed == FALSE & time_dep == "AR1"){
    if(length(model_c_random) == 1 & "e" %in% model_c_random)
    params <- c(params, "b1_te", "b1_tc", "b2_te", "b2_tc") 
  }
  if(length(model_me_random) != 0 & is_me_random_e == FALSE){params <- c(params, "g_e1", "g_e2") }
  if(length(model_mc_random) != 0 & is_mc_random_c == FALSE){params <- c(params, "g_c1", "g_c2") }
  if(length(model_c_random) != 0 & ind_random == FALSE) {params <- c(params, "b1_f", "b2_f") }
  if(ind_random == TRUE | time_dep == "none"){
    if("a1_te" %in% params | "a1_tc" %in% params | "a2_te" %in% params | "a2_tc" %in% params) {
      a_corr_index <- match(c("a1_te", "a1_tc", "a2_te", "a2_tc"), params)
      params <- params[-a_corr_index]
    }
    if("b1_te" %in% params | "b1_tc" %in% params | "b2_te" %in% params | "b2_tc" %in% params) {
      b_corr_index <- match(c("b1_te", "b1_tc", "b2_te", "b2_tc"), params)
      params <- params[-b_corr_index]
    }
  }
  if(type == "MNAR_cost" | type == "MAR") {
    deltae_index <- match(c("delta_e1", "delta_e2"), params)
    params <- params[-deltae_index] 
  }
  if(type == "MNAR_eff" | type == "MAR") {
    deltac_index <- match(c("delta_c1", "delta_c2"), params)
    params <- params[-deltac_index]
  }
  if(type == "MNAR" | type == "MNAR_eff") {
    if(length(model_me_random) != 0 & "e" %in% model_me_random){
      params <- c(params, "d_e1", "d_e2") 
    }
  }
  if(type == "MNAR" | type == "MNAR_cost") {
    if(length(model_mc_random) != 0 & "c" %in% model_mc_random){
      params <- c(params, "d_c1", "d_c2") 
    }
  }
  if(ppc == TRUE) { 
  ...
  gamma_e1 <- modelN1$BUGSoutput$sims.list$gamma_e1
  gamma_e2 <- modelN1$BUGSoutput$sims.list$gamma_e2
  gamma_c1 <- modelN1$BUGSoutput$sims.list$gamma_c1
  gamma_c2 <- modelN1$BUGSoutput$sims.list$gamma_c2
  gamma_e <- list("gamma_e1" = gamma_e1, "gamma_e2" = gamma_e2) 
  gamma_c <- list("gamma_c1" = gamma_c1, "gamma_c2" = gamma_c2) 
  ...
  if(length(model_me_random) != 0 & is_me_random_e == FALSE) { 
    g_e1 <- modelN1$BUGSoutput$sims.list$g_e1
    g_e2 <- modelN1$BUGSoutput$sims.list$g_e2
    g_e <- list("g_e1" = g_e1, "g_e2" = g_e2) 
  }
  if(length(model_mc_random) != 0 & is_mc_random_c == FALSE) { 
    g_c1 <- modelN1$BUGSoutput$sims.list$g_c1
    g_c2 <- modelN1$BUGSoutput$sims.list$g_c2
    g_c <- list("g_c1" = g_c1, "g_c2" = g_c2) 
  }
  ...
  if(ind_fixed == FALSE & ind_time_fixed == FALSE & pc_fixed != 0) {
    beta_f <- modelN1$BUGSoutput$sims.list$beta_f
    if(time_dep == "AR1") {
    beta_te <- modelN1$BUGSoutput$sims.list$beta_te
    beta_tc <- modelN1$BUGSoutput$sims.list$beta_tc
    alpha_te <- modelN1$BUGSoutput$sims.list$alpha_te
  ...
  if(ind_fixed == TRUE & ind_time_fixed == FALSE) {
    ...
    alpha_te <- modelN1$BUGSoutput$sims.list$alpha_te
    alpha_tc <- modelN1$BUGSoutput$sims.list$alpha_tc
    beta <- list("beta" = beta, "beta_te" = beta_te, "beta_tc" = beta_tc)
    alpha <- list("alpha" = alpha, "alpha_te" = alpha_te, "alpha_tc" = alpha_tc)
    }
  }
  ...
  if(length(model_c_random) != 0 & ind_random == FALSE & ind_time_fixed == FALSE) {
    ...
    b1_te <- modelN1$BUGSoutput$sims.list$b1_te
    b2_te <- modelN1$BUGSoutput$sims.list$b2_te
    b1_tc <- modelN1$BUGSoutput$sims.list$b1_tc
    b2_tc <- modelN1$BUGSoutput$sims.list$b2_tc
    a1_te <- modelN1$BUGSoutput$sims.list$a1_te
    a2_te <- modelN1$BUGSoutput$sims.list$a2_te
    ...
    b <- list("b1_f" = b1_f, "b2_f" = b2_f, "b1_te" = b1_te, "b2_te" = b2_te, "b1_tc" = b1_tc, "b2_tc" = b2_tc) 
    } else if(length(model_c_random) != 0 & "e" %in% model_c_random & is_c_random_c == FALSE) {
    b <- list("b1" = b1, "b1_f" = b1_f, "b2" = b2, "b2_f" = b2_f, "b1_te" = b1_te, "b2_te" = b2_te, "b1_tc" = b1_tc, "b2_tc" = b2_tc)
    }
    if(length(model_e_random) != 0) {
      a <- list("a1" = a1, "a2" = a2, "a1_te" = a1_te, "a2_te" = a2_te, "a1_tc" = a1_tc, "a2_tc" = a2_tc) 
    }
  }
  ...
  if(type == "MNAR") {
    delta_e1 <- modelN1$BUGSoutput$sims.list$delta_e1
    delta_e2 <- modelN1$BUGSoutput$sims.list$delta_e2
    delta_c1 <- modelN1$BUGSoutput$sims.list$delta_c1
    delta_c2 <- modelN1$BUGSoutput$sims.list$delta_c2
    delta_e <- list("delta_e1" = delta_e1, "delta_e2" = delta_e2) 
    delta_c <- list("delta_c1" = delta_c1, "delta_c2" = delta_c2) 
    if(length(model_me_random) != 0 & "e" %in% model_me_random) {
      d_e1 <- modelN1$BUGSoutput$sims.list$d_e1
      d_e2 <- modelN1$BUGSoutput$sims.list$d_e2
      d_e <- list("d_e1" = d_e1, "d_e2" = d_e2) 
    }
    if(length(model_mc_random) != 0 & "c" %in% model_mc_random) {
      d_c1 <- modelN1$BUGSoutput$sims.list$d_c1
      d_c2 <- modelN1$BUGSoutput$sims.list$d_c2
      d_c <- list("d_c1" = d_c1, "d_c2" = d_c2) 
    }
  ...
  if(type == "MAR") {
    model_output_jags <- list("summary" = model_sum, "model summary" = modelN1, "mean_effects" = mu_e, "mean_costs" = mu_c, "sd_effects" = s_e, "sd_costs" = s_c, 
                              ...
                              "imputed" = imputed, "loglik" = loglik, "type" = "LONG", "ind_fixed" = ind_fixed, "ind_time_fixed" = ind_time_fixed,"ind_random" = ind_random, "ppc" = ppc, "dist_e" = dist_e, "dist_c" = dist_c)
  } else if(type == "MNAR") {
    model_output_jags <- list("summary" = model_sum, "model summary" = modelN1, "mean_effects" = mu_e, "mean_costs" = mu_c, "sd_effects" = s_e, "sd_costs" = s_c, 
                              "covariate_parameter_effects_fixed" = alpha, "covariate_parameter_costs_fixed" = beta, "missingness_probability_effects" = p_e, 
                              "missingness_parameter_effects_fixed" = gamma_e, "missingness_probability_costs" = p_c, "missingness_parameter_costs_fixed" = gamma_c, 
                              ...
                              "imputed" = imputed, "loglik" = loglik, "type" = "LONG_ec", "ind_fixed" = ind_fixed, "ind_time_fixed" = ind_time_fixed, "ind_random" = ind_random, "ppc" = ppc, "dist_e" = dist_e, "dist_c" = dist_c)
  } else if(type == "MNAR_eff") {
    model_output_jags <- list("summary" = model_sum, "model summary" = modelN1, "mean_effects" = mu_e, "mean_costs" = mu_c, "sd_effects" = s_e, "sd_costs" = s_c, 
                              "covariate_parameter_effects_fixed" = alpha, "covariate_parameter_costs_fixed" = beta, "missingness_probability_effects" = p_e, 
                              "missingness_parameter_effects_fixed" = gamma_e, "missingness_probability_costs" = p_c, "missingness_parameter_costs_fixed" = gamma_c, 
                              ...
                              "imputed" = imputed, "loglik" = loglik, "type" = "LONG_e", "ind_fixed" = ind_fixed, "ind_time_fixed" = ind_time_fixed, "ind_random" = ind_random, "ppc" = ppc, "dist_e" = dist_e, "dist_c" = dist_c)
  } else if(type == "MNAR_cost") {
    model_output_jags <- list("summary" = model_sum, "model summary" = modelN1, "mean_effects" = mu_e, "mean_costs" = mu_c, "sd_effects" = s_e, "sd_costs" = s_c, 
                              "covariate_parameter_effects_fixed" = alpha, "covariate_parameter_costs_fixed" = beta, "missingness_probability_effects" = p_e, 
                              "missingness_parameter_effects_fixed" = gamma_e, "missingness_probability_costs" = p_c, "missingness_parameter_costs_fixed" = gamma_c, 
                              ...
                              "imputed" = imputed, "loglik" = loglik, "type" = "LONG_c", "ind_fixed" = ind_fixed, "ind_time_fixed" = ind_time_fixed, "ind_random" = ind_random, "ppc" = ppc, "dist_e" = dist_e, "dist_c" = dist_c)
  }
