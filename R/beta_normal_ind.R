#' An internal function to write BUGS code for beta-normal model with natural-scaled variables (independence)
#'
...
beta_normal_ind<-function(type)eval.parent(substitute({
  ...
  if(type=="MCAR"){
    ...
    model_string_jags<-prior_change(type=type,dist_e ="beta",dist_c="norm")
  } else if(type=="MAR"){
    ...
    for (j in 1:pc) {#begin beta priors costs
    for(t in 1:2){
  ...
  #include transformations if indicated
  ...
  if(pc==1){
    if(type=="MAR"|type=="MNAR_cov"|type=="MNAR_eff_cov"|type=="MNAR_cost_cov"){
      ...
      prior_beta_c_inter<-"beta_c[t]~dunif(0,10000)"
      ...
      model_string_jags<-gsub("for (j in 1:pc) {#begin beta priors costs", begin_prior_beta, model_string_jags,fixed=TRUE)
  ...
  writeLines(model_string_jags,"beta_normal_ind.txt")
  model_string<-"beta_normal_ind.txt"
  return(model_string)
  }))