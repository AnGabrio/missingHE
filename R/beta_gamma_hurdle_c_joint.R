#' An internal function to write BUGS code for bivariate beta-gamma with an hurdle model for the costs (joint)
#'
#' This function writes in the current WD a txt file 
#' for the bivariate joint beta-gamma with an hurdle model for the costs.
...
beta_gamma_hurdle_c_joint<-function(type)eval.parent(substitute({
  ...
  if(type=="SCAR"){
    ...
    #obtain mean and sd
    tau_e1[i]<-(mu_e1[i]*(1-mu_e1[i])/pow(s_e1,2)-1)
    tau_c1[i]<-mu_c1[i]/pow(s_c1[d_cost1[i]+1],2)
    #mean regression
    log(mu_c1[i])<-inprod(X1_c[i,],beta_c1[,d_cost1[i]+1])+theta1[d_cost1[i]+1]*(eff1[i]-mu_e[1])
    logit(mu_e1[i])<-inprod(X1_e[i,],beta_e1[])    
    ...
    #obtain mean and sd
    tau_e2[i]<-(mu_e2[i]*(1-mu_e2[i])/pow(s_e2,2)-1)
    tau_c2[i]<-mu_c2[i]/pow(s_c2[d_cost2[i]+1],2)
    #mean regression
    log(mu_c2[i])<-inprod(X2_c[i,],beta_c2[,d_cost2[i]+1])+theta2[d_cost2[i]+1]*(eff2[i]-mu_e[2])
    logit(mu_e2[i])<-inprod(X2_e[i,],beta_e2[])    
    #structural values mechanism
    ...
    nu_e[1]<-ilogit(inprod(mean_cov_e1[],beta_e1[]))
    nu_e[2]<-ilogit(inprod(mean_cov_e2[],beta_e2[]))
    ...
    s_c1[1]~dunif(0,1000)         
    s_e1~dunif(0,sqrt(nu_e[1]*(1-nu_e[1])))
    s_c2[1]~dunif(0,1000)         
    s_e2~dunif(0,nu_e[2]*(1-nu_e[2]))
    s_c1[2]<-sdc         
    s_c2[2]<-sdc
    ...
    s_e1~dunif(0,sqrt(nu_e[1]*(1-nu_e[1])))
    s_c2[1]~dunif(0,1000)         
    s_e2~dunif(0,sqrt(nu_e[2]*(1-nu_e[2])))
    s_c1[2]<-sdc         
    s_c2[2]<-sdc
  ...
  #call prior_change function to make changes to prior values and distributions if prior inputs in hurdle are provided
  model_string_jags<-prior_hurdle(type=type,dist_e ="beta",dist_c="gamma")
      #assign name to model file to be called by run_jags or run_bugs
      writeLines(model_string_jags,"beta_gamma_hurdle_c_ind.txt")
      model_string<-"beta_gamma_hurdle_c_ind.txt"
  return(model_string)
}))
