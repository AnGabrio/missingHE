...
#' @param model.se A formula expression in conventional R linear modelling syntax.  The response must be indicated with the 
...
#' @param model.sc A formula expression in conventional R linear modelling syntax. The response must be indicated with the 
...
hurdle<-function(data,model.eff,model.cost,model.se=se~1,model.sc=sc~1,se=1,sc=0,
  ...
  #need to specify type of structural values mechanism among those available
  ...
  data_read<-data_read_hurdle(data=data,model.eff=model.eff,model.cost=model.cost,model.se=model.se,model.sc=model.sc,se=se,sc=sc,type=type)
  ...
  #covariates for se and sc models (vectors of 1s if no variables)
  ...
  sde<-se.prior
  sdc<-sc.prior
  #stop if multiple values provided
  if(length(sde)!=1|length(sdc)!=1){stop("Single value priors on std for structural values must be ")}
  ...
  #check if indicator variables provided by user and replace with the ones from the data (MNAR)
  ...
  if(is.null(d_c)==FALSE){
    ...
    if(is.null(sc)==TRUE & is.null(se)==FALSE){
      data_set<-list("effects"=data_read$raw_effects,"costs"=data_read$raw_costs,"N in reference arm"=N1,"N in comparator arm"=N2,
                     ...
                     "covariates_structural_effects"=data_read$covariates_structural_effects,
                     "structural_effects"=data_read$structural_effects)
    }else if(is.null(sc)==FALSE & is.null(se)==TRUE){
      data_set<-list("effects"=data_read$raw_effects,"costs"=data_read$raw_costs,"N in reference arm"=N1,"N in comparator arm"=N2,
                     "N observed in reference arm"=N1_cc,"N observed in comparator arm"=N2_cc,"N missing in reference arm"=N1_mis,"N missing in comparator arm"=N2_mis,
                     "covariates_effects"=data_read$covariates_effects,"covariates_costs"=data_read$covariates_costs,
                     "covariates_structural_costs"=data_read$covariates_structural_costs,
                     "structural_costs"=data_read$structural_costs)
    }else if(is.null(sc)==FALSE & is.null(se)==FALSE){
      data_set<-list("effects"=data_read$raw_effects,"costs"=data_read$raw_costs,"N in reference arm"=N1,"N in comparator arm"=N2,
                     "N observed in reference arm"=N1_cc,"N observed in comparator arm"=N2_cc,"N missing in reference arm"=N1_mis,"N missing in comparator arm"=N2_mis,
                     "covariates_effects"=data_read$covariates_effects,"covariates_costs"=data_read$covariates_costs,
